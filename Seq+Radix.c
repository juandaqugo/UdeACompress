...
void ompRadix(uint32_t size, uint8_t *cadena, uint32_t *Indexes, uint8_t nthreads);
void seqRadix(uint32_t size, uint8_t *cadena, uint32_t *Indexes, uint8_t *HU, uint32_t Iter, int64_t inf, int64_t sup, uint32_t *ID);
void Seq_lsd_radix_sort_(uint32_t size, uint8_t *cadena, uint32_t *Indexes, int64_t inf, int64_t sup);
void SelectValidCharsMsc (uint8_t *InputText, uint32_t size, uint32_t Iter, uint32_t *Indexes, uint8_t *OutInt, uint8_t *HU, uint32_t *ID, int64_t inf , int64_t sup);
uint32_t GroupBuilder (uint8_t *InputText, uint32_t size, uint32_t Iter, uint32_t *Indexes, uint8_t *OutInt, int64_t *frontera, uint8_t *HU, uint32_t *ID, int64_t inf, int64_t sup);
int main(int argc, char* argv[]){
	...
	//printf("inicia\n"); fflush(stdout);
	ompRadix(size, cadena, Indexes, nthreads);
...
void gen_data(uint8_t *cadena, uint32_t size){
	//uint8_t cad[]={99,98,100,98,97,99,98,97,98,98,100,97,97,98,99};
	uint32_t i;
	uint8_t num;
	srand(time(NULL)); 
	for (i=0;i<size;i++){
...
void ompRadix(uint32_t size, uint8_t *cadena, uint32_t *Indexes, uint8_t nthreads){
	...
	//	#pragma omp parallel for num_threads(nthreads) shared(frontera, cadena, size, band, Indexes)
		for(x=0;x<subgrupos;x++){
			if ( (frontera[x+1]-(frontera[x]+1)>SEQ_THRESHOLD) ){
		...
		//		gettimeofday(&t1, NULL);	
				...
				#ifdef DEBUG2 
				for(int i=0;i<size;i++){printf("Out omp=%d\n", OutInt[i]);/*	printf("Data pos Segun Ind[%u]=%u,  %u\n ",i,Indexes[i],  cadena[Indexes[i]]);*/}
				for(int i=0;i<size;i++){printf("Indexes omp=%d\n", Indexes[i]);}		
				#endif
				omp_lsd_radix_sortInd(size, OutInt, Indexes, frontera[x]+1, frontera[x+1]+1, nthreads);
				#ifdef DEBUG2
				for(int i=0;i<size;i++){printf("___Indexesomp___=%d\n", Indexes[i]);}
				#endif
		/*		gettimeofday(&t2, NULL);
  				elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000.0;      // sec to ms
  				elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0;   // us to ms}
			...
			}else if(((frontera[x+1]-(frontera[x]+1)) <= SEQ_THRESHOLD) && ((frontera[x+1]-(frontera[x]+1)) > 0)){
					//uint32_t lim = frontera[x+1]-(frontera[x]+1);
				/*	if(x==0) */seqRadix(size, cadena, Indexes, HU, Iter, frontera[x], frontera[x+1], ID);
			//		else seqRadix(frontera[x+1]-(frontera[x]), cadena, Indexes, HU, Iter, frontera[x]-1, frontera[x+1], ID);
//break;
			//		printf("acaba x=%d\n", x);
			}else {
	...
	//		#pragma omp critical
		...
		Iter++;
		subgrupos = GroupBuilder (cadena, size, Iter, Indexes, OutInt, frontera, HU, ID, 0, size-1);
		#ifdef DEBUG2
		for(int i=0;i<size;i++){printf("___Frontera___=%d\n", frontera[i]);}
		#endif
//		if(Iter==2)break;
	//	printf("subgrupos= %d\n", subgrupos);
	}while (!band);
...
/*		printf("error ID\n");
	...
	frontera[0] = inf;
//	if(inf!=-1)frontera[0]=inf-1;
//	printf("frontera %d, sup=%d\n", frontera[0], sup);
...
//	uint32_t Iter=0;
	...
	Iter++;
	GroupBuilder (cadena, size,Iter, Indexes, OutInt, frontera, HU, ID);*/
//	frontera [subgrupos] = size;
	do{
		//subgrupos = 1;
		//frontera[subgrupos] = size-1;
	//	SelectValidCharsMsc(cadena, size, Iter, Indexes, OutInt, HU, ID, 0, size-1);/*ultimo cambio*/
		//subgrupos = GroupBuilder (cadena, size, Iter, Indexes, OutInt, frontera, HU, ID);
		frontera [subgrupos] = sup;//size-1;
	//	printf("frontera[%d]=%d", subgrupos, sup);
		band=1;
		for(x=0;x<subgrupos;x++){
			#ifdef DEBUG2
			printf("frontera[%d]=%d", x+1,frontera[x+1]);
			#endif
			...
			//	gettimeofday(&t1, NULL);	
				...
				#ifdef DEBUG2
				for(int i=0;i<size;i++){printf("Out_Seq[%d] = %d\n",i, OutInt[i]); }
				for(int i=0;i<size;i++){printf("Indexes_seq[%d] = %d\n",i, Indexes[i]); }
				#endif
			//	printf("OutInt %s\n", OutInt);	
				Seq_lsd_radix_sort_(size, OutInt, Indexes, frontera[x]+1, frontera[x+1]);
				#ifdef DEBUG2
				for(int i=0;i<size;i++){printf("__Indexes_seq__[%d] = %d\n",i, Indexes[i]); }
				#endif
			/*	gettimeofday(&t2, NULL);
  				elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000.0;      // sec to ms
  				elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0;   // us to ms}
		...
		Iter++;
		subgrupos = GroupBuilder (cadena, size, Iter, Indexes, OutInt, frontera, HU, ID, inf+1, sup);
		#ifdef DEBUG2
		for(int i=0;i<size;i++){printf("___FronteraSEQ___=%d\n", frontera[i]);}
		printf("subgrupos = %d\n", subgrupos);
		#endif
	//	if(Iter==4)break;
		//printf("%d\n", Iter);
...
void SelectValidCharsMsc (uint8_t *InputText, uint32_t size, uint32_t Iter, uint32_t *Indexes, uint8_t *OutInt, uint8_t *HU, uint32_t *ID, int64_t inf , int64_t sup){
	...
	//printf("SIZE=%d\n", size);
	   for (It=inf;It<=sup;++It){  //Para cada uno d elos caracterres validos    
		...
		#ifdef DEBUG2
		printf("ValidChar = %d, inf=%d, size=%d, sup=%d\n", ValidChar, inf, size, sup);
		#endif
      		if(!(ValidChar >= size)){       //PAdding 
